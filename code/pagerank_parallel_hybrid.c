...
int main(int argc, char *argv[]) {
    ...
    double **local_contrib_thread = (double**) malloc(num_threads * sizeof(double*));
    if (!local_contrib) {
        fprintf(stderr, "Failed to allocate local_contrib.\n");
        exit(EXIT_FAILURE);
    }
    for (int t = 0; t < num_threads; t++) {
        local_contrib_thread[t] = (double*) calloc(node_count, sizeof(double));
        if (!local_contrib_thread[t]) {
