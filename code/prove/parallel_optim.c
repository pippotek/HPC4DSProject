...
int main(int argc, char *argv[]) {
    ...
    int *in_degree = (int *)calloc(node_count, sizeof(int));
    int *global_in_degree = (int *)calloc(node_count, sizeof(int));
    for(int i = 0;i< node_count;i++){
        in_degree[i] = 0;
        global_in_degree[i] = 0;
    }

    for(int i = 0;i<local_edge_count;i++){
        in_degree[local_edges[i].to]++;
    }

    MPI_Allreduce(in_degree, global_in_degree, node_count, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

    int *map_array = (int *)malloc(sizeof(int) * node_count);
    for(int i = 0;i<node_count;i++){
        map_array[i] = 0;
    }
    ...
    for(int i = 1;i<node_count;i++){
        if(global_in_degree[i] != 0 || out_degree[i] != 0)
            map_array[i] = i - static_counter;
        else
            static_counter++;
    }

    int changing_nodes = node_count - static_counter;
    printf("Found %d static nodes with %d changing nodes\n", static_counter, changing_nodes);
    ...
    double *local_contrib = (double *) malloc(sizeof(double) * changing_nodes);
    double *global_contrib = (double *) malloc(sizeof(double) * changing_nodes);
    ...
    for (int iter = 0; iter < MAX_ITER; iter++) {
        ...
        for (int i = 0; i < local_edge_count; i++) {
            ...
            if (out_degree[from] > 0) {
                #pragma omp atomic
                local_contrib[map_array[to]] += DAMPING_FACTOR * rank_vals[from] / out_degree[from];
            }
        }
        /* 4) Sum all the per-edge contributions across processes */
        MPI_Allreduce(local_contrib, global_contrib, changing_nodes, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

        /* 5) Combine the base, dangling, and edge contributions to update rank */
        #pragma omp parallel for
        for (int i = 0; i < node_count; i++) {
            temp_rank[i] = base_rank + dangling_contrib + global_contrib[map_array[i]];
        }
        ...
        for(int i = 0;i<changing_nodes;i++){
            local_contrib[i] = 0;
        }
